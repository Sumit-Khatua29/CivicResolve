...
import { FaEdit, FaTrash, FaCheckCircle, FaExclamationCircle, FaClock, FaTimesCircle, FaMapMarkedAlt, FaList } from "react-icons/fa";
...
const MapBounds = ({ issues }) => {
    ...
    useEffect(() => {
        if (issues.length > 0) {
            const bounds = L.latLngBounds(issues.map(issue => [parseFloat(issue.latitude), parseFloat(issue.longitude)]));
            if (bounds.isValid()) {
...
const CitizenDashboard = () => {
  ...
  const [stats, setStats] = useState({ total: 0, resolved: 0, verified: 0, pending: 0, rejected: 0 });
  ...
  const fetchIssues = async () => {
    try {
      ...
      setStats({
          ...
          pending: data.filter(i => i.status === 'PENDING' || i.status === 'IN_PROGRESS').length,
      ...
      <motion.div whileHover={{ y: -5 }}>
          <Card className={`border-0 shadow-sm mb-4 bg-white`}>
              <Card.Body className="d-flex align-items-center">
                  <div className={`rounded-circle bg-${variant} bg-opacity-10 p-3 me-3 text-${variant}`}>
                      {icon}
                  </div>
                  <div>
  ...
  return (
    ...
    <Container className="citizen-dashboard-container">
      ...
      {showMap && (
          ...
          >
               <div className="map-view-container border rounded overflow-hidden">
                  <MapContainer center={defaultCenter} zoom={5} className="map-container-full">
                    <TileLayer
                                ...
                                >
                                    <Popup>
                                        <div className="popup-content">
                                            <h6 className="mb-1 fw-bold">{issue.category}</h6>
                                            <p className="small mb-1 text-muted">{issue.description.substring(0, 50)}...</p>
                                            <Badge bg={
                                                issue.status === "RESOLVED" ? "success" :
                                                issue.status === "VERIFIED" ? "info" :
                                                issue.status === "IN_PROGRESS" ? "warning" :
                                                issue.status === "REJECTED" ? "danger" : "secondary"
                                            }>
                                                {issue.status}
                                            </Badge>
                                            <div className="mt-2">
                                                <Link to={`/edit-issue/${issue.id}`} className="text-decoration-none small">View Details</Link>
                                            </div>
                                        </div>
                                    </Popup>
                                </Marker>
      ...
      {issues.length === 0 ? (
          <p className="text-muted text-center py-5">You haven't reported any issues yet.</p>
      ) : (
        <Table hover responsive className="table-premium bg-transparent">
            ...
            {issues.map((issue, index) => (
                ...
                <td>
                    {issue.imagePath ? (
                        <div className="mt-1">
                            <a href={issue.imagePath} target="_blank" rel="noopener noreferrer">View Image</a>
                        </div>
                    ) : '-'}
                </td>
                <td>
