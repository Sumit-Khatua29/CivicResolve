  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === "ROLE_ADMIN") { 
          ...
          // Optionally logout or handle this case
      }
    } catch (err) {
            ...
            >
                <Card className="glass-card border-0 rounded-4 overflow-hidden text-dark shadow-lg">
                    <Card.Body className="p-4 p-md-5">
                        ...
                        {error && <Alert variant="danger" className="border-0 bg-danger text-white fill-danger">{error}</Alert>}
                        
                        <Form onSubmit={handleSubmit}>
                            <Form.Group className="mb-3" controlId="formBasicUsername">
                                <Form.Label className="fw-semibold small text-uppercase admin-auth-label">Username</Form.Label>
                                <InputGroup>
                            ...
                            <Form.Group className="mb-4" controlId="formBasicPassword">
                                <Form.Label className="fw-semibold small text-uppercase admin-auth-label">Password</Form.Label>
                                <InputGroup>
                                    <InputGroup.Text className="bg-white text-primary border-end-0">
                                        <FaLock />
                                    </InputGroup.Text>
                            ...
                            <Button variant="warning" type="submit" className="w-100 btn-primary-custom rounded-pill fw-bold text-white shadow">
                                Login to Dashboard
                            </Button>
                        </Form>
                        ...
                        <div className="text-center mt-4 border-top pt-3">
                            <p className="mb-0 text-muted small">
                                Not an admin? <Link to="/login" className="text-primary fw-bold text-decoration-none">Citizen Login</Link>
                            </p>
                        </div>
                    </Card.Body>
                </Card>
